      ...
      {parts.map((part, i) => {
        ...
        return (
          <React.Fragment key={i}>
            ...
            <button onClick={() => onNavigate(cumulativePath)} className="text-gray-500 hover:text-violet-500 transition-colors whitespace-nowrap hover:underline underline-offset-4 decoration-violet-500/30">{part}</button>
          </React.Fragment>
        );
      })}
      <ChevronRight size={10} className="text-gray-400 shrink-0" />
        ...
        return (
          <div key={node.fullPath} className="relative group/item">
            {(!node.page || hasChildren) && (
              <div className="my-1">
                <button onClick={() => toggle(name)} className={`w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 relative overflow-hidden group/btn ${isFolderOpen ? 'bg-gray-100 dark:bg-white/5' : 'hover:bg-gray-50 dark:hover:bg-white/5'}`} style={{ paddingLeft: `${depth * 16 + 12}px` }}>
            ...
            {node.page && !hasChildren && (
              <div className="my-0.5 relative">
                <button onClick={() => onSelect(node.page!)} style={{ paddingLeft: `${depth * 16 + 28}px` }} className={`w-full text-left text-sm truncate py-2 pr-3 rounded-lg transition-all duration-300 flex items-center gap-3 relative ${isSelected ? 'text-gray-900 dark:text-white bg-violet-50 dark:bg-violet-500/10 border-l-2 border-violet-500' : 'text-gray-500 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5 border-l-2 border-transparent'}`}>
...
const WikiWelcome: React.FC<{ pages: WikiPageMetadata[]; onShowMasonry: () => void; onSelect: (page: WikiPageMetadata) => void; }> = ({ pages, onShowMasonry, onSelect }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-start py-8 px-4 md:p-12 max-w-5xl mx-auto animate-in fade-in zoom-in duration-500 w-full overflow-y-auto custom-scrollbar">
      <div className="text-center mb-8 md:mb-10 relative">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-base md:text-xl max-w-2xl mx-auto leading-relaxed mb-6 md:mb-8">Mon environnement de travail partagé avec la communauté. Une démarche professionnelle pour diffuser mes connaissances, sans prétention.</p>
        <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-4 mb-10 md:mb-12">
          <button onClick={onShowMasonry} className="px-6 py-3 bg-violet-600 hover:bg-violet-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-violet-500/25 active:scale-95 flex items-center justify-center gap-2"><Layers size={18} />Tout Parcourir</button>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 w-full max-w-2xl mb-12 md:mb-16">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white dark:bg-[#13131a]/50 border border-gray-200 dark:border-white/5 rounded-2xl p-4 sm:p-6 flex flex-col items-center justify-center gap-2 sm:gap-3 shadow-sm group hover:border-violet-500/30 transition-all duration-300">
            <div className="p-2 sm:p-3 rounded-xl bg-violet-500/10 text-violet-500 group-hover:scale-110 transition-transform"><stat.icon size={20} className="sm:w-6 sm:h-6" /></div>
            <div className="text-center"><div className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white">{stat.value}</div><div className="text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-widest font-bold mt-0.5 sm:mt-1">{stat.label}</div></div>
          </div>
        ))}
      </div>
      <div className="w-full max-w-3xl text-left">
        <h3 className="text-xs sm:text-sm font-bold text-gray-400 uppercase tracking-widest mb-4 md:mb-6 flex items-center gap-2"><Activity size={16} className="text-violet-500" />Mises à jour récentes</h3>
        <div className="space-y-2 md:space-y-3">
          {lastUpdated.map(page => (
            <button key={page.id} onClick={() => onSelect(page)} className="w-full text-left p-3 md:p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:border-violet-500/50 hover:bg-white dark:hover:bg-white/10 transition-all flex items-center justify-between group">
              <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
                <div className="p-1.5 md:p-2 rounded-lg bg-white dark:bg-[#0a0a0f] border border-gray-200 dark:border-white/10 group-hover:text-violet-500 transition-colors shrink-0"><FileText size={14} className="md:w-4 md:h-4" /></div>
                <div className="truncate"><div className="text-xs md:text-sm font-bold text-gray-900 dark:text-white truncate">{page.title}</div><div className="text-[9px] md:text-[10px] text-gray-500 truncate uppercase tracking-wider">{page.category}</div></div>
              </div>
              <div className="text-[9px] md:text-[10px] text-gray-400 font-mono whitespace-nowrap ml-2">{new Date(page.updated_at).toLocaleDateString('fr-FR')}</div>
            </button>
          ))}
        </div>
      </div>
...
const WikiMasonry: React.FC<{ pages: WikiPageMetadata[]; onSelect: (page: WikiPageMetadata) => void; onClose: () => void; }> = ({ pages, onSelect, onClose }) => {
  const groupedPages = React.useMemo(() => {
    ...
    return Object.keys(groups).sort().reduce((acc, key) => { acc[key] = groups[key].sort((a, b) => a.title.localeCompare(b.title)); return acc; }, {} as Record<string, WikiPageMetadata[]>);
  }, [pages]);
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-white/50 dark:bg-[#0a0a0f]/50 backdrop-blur-3xl">
      <div className="p-6 border-b border-gray-200 dark:border-white/5 flex items-center justify-between sticky top-0 z-20 bg-inherit backdrop-blur-md">
        <div><h2 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-3"><Layers className="text-violet-500" />Bibliothèque Complète</h2><p className="text-xs text-gray-500 font-bold uppercase tracking-widest mt-1">{pages.length} Notes Indexées</p></div>
        <button onClick={onClose} className="p-2 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-colors"><X size={24} /></button>
      </div>
      <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 custom-scrollbar space-y-12">
        <div className="flex flex-wrap gap-2 pb-6 border-b border-gray-200 dark:border-white/5">
          {Object.keys(groupedPages).map(category => (
            <button key={category} onClick={() => { const element = document.getElementById(`category-${category.toLowerCase().replace(/\s+/g, '-')}`); element?.scrollIntoView({ behavior: 'smooth' }); }} className="px-3 py-1.5 rounded-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-violet-500 hover:border-violet-500/50 hover:bg-violet-500/5 transition-all flex items-center gap-2 group">
              <span className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-700 group-hover:bg-violet-500 transition-colors"></span>{category}
            </button>
          ))}
        </div>
        {Object.entries(groupedPages).map(([category, categoryPages]) => (
          <div key={category} id={`category-${category.toLowerCase().replace(/\s+/g, '-')}`} className="space-y-6 scroll-mt-28">
            <div className="flex items-center gap-3 border-b border-gray-200 dark:border-white/5 pb-4">
              <div className="p-2 rounded-xl bg-gray-100 dark:bg-white/5 shadow-inner">{getCategoryIcon(category)}</div>
              <div><h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">{category}</h3><p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{categoryPages.length} {categoryPages.length > 1 ? 'Documents' : 'Document'}</p></div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {categoryPages.map(page => (
                <motion.button key={page.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} onClick={() => onSelect(page)} className="group text-left p-4 sm:p-6 rounded-2xl bg-white dark:bg-[#13131a]/80 border border-gray-200 dark:border-white/5 hover:border-violet-500/50 hover:shadow-2xl hover:shadow-violet-500/10 transition-all flex flex-col gap-3 sm:gap-4 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-0 group-hover:h-full bg-violet-600 transition-all duration-300" />
                  <div className="flex items-center justify-between">
                    <div className="p-1.5 sm:p-2 rounded-lg bg-gray-50 dark:bg-black/20 text-gray-400 group-hover:text-violet-500 transition-colors shrink-0"><FileText size={16} /></div>
                    <div className="text-[9px] sm:text-[10px] font-mono text-gray-400 uppercase tracking-tighter">{new Date(page.updated_at).toLocaleDateString('fr-FR')}</div>
                  </div>
                  <div className="min-w-0"><h4 className="font-bold text-sm sm:text-base text-gray-900 dark:text-white group-hover:text-violet-500 transition-colors line-clamp-2 leading-snug">{page.title}</h4><div className="text-[9px] sm:text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1.5 sm:mt-2 truncate">{page.category}</div></div>
                  <div className="mt-auto pt-3 sm:pt-4 border-t border-gray-100 dark:border-white/5 flex flex-wrap gap-1 md:gap-1.5">{page.tags?.slice(0, 3).map(tag => (<span key={tag} className="text-[8px] sm:text-[9px] bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded text-gray-500">#{tag}</span>))}</div>
                </motion.button>
              ))}
            </div>
          </div>
...
const Backlinks: React.FC<{ currentPage: WikiPageType; onSelect: (page: WikiPageMetadata) => void }> = ({ currentPage, onSelect }) => {
  ...
  return (
    <div className="mt-12 md:mt-16 pt-6 md:pt-8 border-t border-gray-200 dark:border-white/5 text-left">
      <h3 className="text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest mb-4 md:mb-6 flex items-center gap-2"><CornerDownRight size={12} className="md:w-3.5 md:h-3.5" />Backlinks {isLoading ? '...' : `(${backlinks.length})`}</h3>
      {isLoading ? (
        <div className="flex gap-4">{[1, 2].map(i => <div key={i} className="h-16 flex-1 bg-gray-100 dark:bg-white/5 animate-pulse rounded-xl"></div>)}</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
          {backlinks.map(link => (
            <button key={link.id} onClick={() => onSelect(link)} className="text-left p-3 md:p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:border-violet-500/30 transition-all group">
              <div className="text-xs md:text-sm font-bold text-gray-900 dark:text-white group-hover:text-violet-500 transition-colors truncate">{link.title}</div>
...
export const WikiPage: React.FC = () => {
  ...
  useEffect(() => { if (slug) { setIsMasonryView(false); } }, [slug]);

  const handlePageSelect = (page: WikiPageMetadata) => { navigate(`/wiki/${page.slug}`); };
  ...
  const calculateScore = (page: WikiPageMetadata, query: string) => {
    ...
    let score = 0;
    if (page.title?.toLowerCase().includes(q)) { score += 100; if (page.title.toLowerCase() === q) score += 200; }
  ...
  const filteredPages = React.useMemo(() => {
    return pages.map(p => ({ page: p, score: calculateScore(p, searchQuery) })).filter(item => item.score > 0).sort((a, b) => b.score - a.score).map(item => item.page);
  }, [pages, searchQuery]);
  ...
  const MarkdownComponents = {
    ...
    img: ({ src, alt }: any) => (<div className="my-6 sm:my-8 rounded-xl sm:rounded-2xl overflow-hidden border border-gray-200 dark:border-white/10 shadow-lg group cursor-zoom-in" onClick={() => setSelectedImage(src)}><img src={src} alt={alt} className="w-full hover:scale-[1.02] transition-transform duration-500" />{alt && <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-50 dark:bg-white/5 text-[9px] sm:text-[10px] text-gray-500 font-medium italic border-t border-gray-200 dark:border-white/10 text-center">{alt}</div>}</div>),
  ...
  useEffect(() => {
    ...
    filteredPages.forEach(page => {
      ...
      parts.forEach((part, index) => {
        if (!currentLevel[part]) currentLevel[part] = { name: part, fullPath: parts.slice(0, index + 1).join('/'), children: {}, isOpen: false, count: 0 };
        currentLevel[part].count++;
        if (index === parts.length - 1) currentLevel[part].children[page.title] = { name: page.title, fullPath: page.slug, children: {}, page: page, isOpen: false, count: 1 };
  ...
  return (
    <>
      <SEOHead title={selectedPage ? `${selectedPage.title} | Wiki` : 'Wiki & Knowledge Base'} description={selectedPage ? `Note sur ${selectedPage.title} dans le wiki.` : 'Base de connaissances'} />
      ...
      <div className="lg:hidden fixed top-16 left-0 right-0 z-30 bg-white/80 dark:bg-[#0a0a0f]/80 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 px-4 py-3 flex items-center gap-3">
        <div className="flex-1 relative group">
          <div className="relative flex items-center bg-gray-100 dark:bg-white/5 border border-transparent focus-within:border-violet-500/50 rounded-xl transition-all">
            ...
            <input type="text" placeholder="Rechercher dans le wiki..." className="w-full bg-transparent py-2.5 pl-2 pr-4 text-sm text-gray-900 dark:text-gray-200 focus:outline-none placeholder-gray-500 font-medium" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); if (e.target.value && !isMasonryView && !selectedPage) setIsMasonryView(true); }} />
            {searchQuery && (<button onClick={() => setSearchQuery('')} className="p-1.5 mr-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X size={14} /></button>)}
          </div>
        </div>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2.5 bg-violet-600/10 text-violet-600 rounded-xl border border-violet-600/20 active:scale-95 transition-all">{isSidebarOpen ? <X size={20} /> : <Menu size={20} />}</button>
      </div>

      <div className="min-h-screen bg-background flex pt-[7.5rem] lg:pt-24 pb-4 md:pb-6 px-2 md:px-8 gap-0 md:gap-6 overflow-hidden transition-colors duration-300">
        {isSidebarOpen && (<div className="lg:hidden fixed inset-0 z-30 bg-black/60 backdrop-blur-sm" onClick={() => setIsSidebarOpen(false)} />)}
        <aside className={`fixed inset-y-0 left-0 z-40 w-full sm:w-80 lg:relative lg:inset-auto lg:w-80 lg:block bg-white dark:bg-[#0a0a0f] lg:bg-surface/90 lg:dark:bg-[#13131a]/80 backdrop-blur-xl border-r lg:border border-gray-200 dark:border-white/10 lg:rounded-2xl flex flex-col transition-all duration-500 shadow-2xl ${isSidebarOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 lg:translate-x-0 lg:opacity-100 lg:w-80'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            {isLoadingList ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3"><div className="w-8 h-8 border-2 border-violet-500/30 border-t-violet-500 rounded-full animate-spin"></div><span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Indexation...</span></div>
            ) : Object.keys(tree).length === 0 ? (
              <div className="text-center py-12 text-gray-500"><p>Aucune donnée.</p></div>
            ) : (
        ...
        <main className="flex-1 bg-white dark:bg-[#0a0a0f] lg:bg-surface/50 lg:dark:bg-[#13131a]/40 lg:backdrop-blur-md border-x lg:border border-gray-200 dark:border-white/5 lg:rounded-2xl overflow-hidden relative shadow-xl flex flex-col lg:flex-row">
          {slug ? (
            isLoadingContent ? (
              <div className="flex-1 flex flex-col items-center justify-center p-12 gap-4"><div className="w-12 h-12 border-4 border-violet-500/20 border-t-violet-500 rounded-full animate-spin"></div><div className="text-gray-500 font-bold uppercase tracking-[0.2em] text-xs">Chargement de la note...</div></div>
            ) : selectedPage ? (
              <div className="flex-1 flex flex-row overflow-hidden h-full">
                <div className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-8 md:p-12 scroll-smooth relative">
                  <motion.div key={selectedPage.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="max-w-4xl mx-auto relative z-10">
                    <Breadcrumbs category={selectedPage.category} title={selectedPage.title} onNavigate={(path) => { navigate('/wiki'); setSearchQuery(path); if (path !== '') setIsMasonryView(true); else setIsMasonryView(false); }} />
