...
import { 
  ...
  Layers, FolderOpen, CornerDownRight, Check, Copy // ✅ AJOUT : Check & Copy
} from 'lucide-react';
...
// --- COMPOSANT CODE BLOCK AMÉLIORÉ ---
const CodeBlock = ({ children, className, ...props }: any) => {
  const [copied, setCopied] = useState(false);
  
  // Extraction du langage depuis la classe (ex: language-bash)
  ...
  // Si pas de langage spécifié ou code inline, rendu simple
  if (!match) {
    return (
  ...
  // Rendu "Terminal Intelligent" pour les blocs de code
  return (
...
const FileTree: React.FC<{ 
  ...
  return (
    <div className="flex flex-col relative">
      {depth > 0 && (
        <div className="absolute left-[11px] top-0 bottom-0 w-px bg-white/5 border-l border-dashed border-white/20" />
      )}
          ...
          <div key={node.fullPath} className="relative">
            ...
            {(!node.page || hasChildren) && (
              <div className="my-1">
                <button 
                  onClick={() => toggle(name)}
                  className={`w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 group relative overflow-hidden
                    ${isFolderOpen ? 'bg-white/5' : 'hover:bg-white/5'}
                  `}
                  style={{ paddingLeft: `${depth * 16 + 12}px` }}
                >
                  {depth > 0 && (
                    <CornerDownRight className="absolute text-white/20 w-3 h-3" style={{ left: `${depth * 16 - 6}px` }} />
                  )}

                  <span className={`text-gray-500 transition-transform duration-300 ${isFolderOpen ? 'rotate-90 text-violet-400' : ''}`}>
                    <ChevronRight size={14} />
                  </span>
                  
                  <span className={`transition-colors duration-300 ${isFolderOpen ? 'text-violet-300' : 'text-gray-400 group-hover:text-gray-200'}`}>
                    {isFolderOpen ? <FolderOpen size={15} /> : <Folder size={15} />}
                  </span>
                  
                  <span className={`text-xs font-bold uppercase tracking-wider truncate transition-colors ${isFolderOpen ? 'text-white' : 'text-gray-400'}`}>
                    {name}
                  </span>
                </button>
            ...
            {node.page && !hasChildren && (
              <div className="my-1 relative">
                 {depth > 0 && (
                    <CornerDownRight className="absolute top-3 text-white/20 w-3 h-3" style={{ left: `${depth * 16 + 4}px` }} />
                  )}
                <button
                  ...
                  className={`w-full text-left text-sm truncate py-2 pr-3 rounded-lg transition-all duration-300 flex items-center gap-3 relative
                    ${isSelected 
                      ? 'text-white bg-gradient-to-r from-violet-600/20 to-transparent border-l-2 border-violet-500' 
                ...
                >
                  ...
                  {isSelected && (
                    <motion.div 
                      layoutId="activeGlow"
                      className="absolute inset-0 bg-violet-500/5 rounded-lg -z-10"
                      initial={false}
  ...
  const MaintenanceState = () => (
    <div className="flex flex-col items-center justify-center h-[60vh] text-center p-8 border-2 border-dashed border-white/5 rounded-3xl bg-[#13131a]/30 backdrop-blur-sm">
      ...
      <p className="text-gray-400 max-w-md leading-relaxed text-lg">
        Cette donnée est référencée dans le cortex mais le contenu n'a pas encore été synchronisé.
      </p>
      <div className="mt-10 flex gap-4">
        <span className="px-4 py-1.5 bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 rounded-full text-xs font-mono tracking-widest">
          STATUS: PENDING
        </span>
      </div>
    </div>
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0a0a0f] flex pt-24 pb-6 px-4 md:px-8 gap-6 overflow-hidden">
        ...
        <button 
          ...
          className="lg:hidden fixed bottom-6 right-6 z-50 bg-violet-600 p-4 rounded-full shadow-[0_0_30px_rgba(139,92,246,0.4)] text-white hover:bg-violet-500 transition-all active:scale-95"
        >
        ...
        {/* --- SIDEBAR FLOTTANTE --- */}
        <aside className={`
          fixed inset-y-24 left-4 lg:left-8 z-40 w-80 lg:relative lg:inset-auto lg:w-96 lg:block
          bg-[#13131a]/80 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col 
          transition-all duration-500 ease-[cubic-bezier(0.25,0.8,0.25,1)] shadow-2xl
          ${isSidebarOpen ? 'translate-x-0 opacity-100' : '-translate-x-[120%] opacity-0 lg:translate-x-0 lg:opacity-100 lg:w-96'}
        `}>
          {/* Header Sidebar */}
          <div className="p-6 border-b border-white/5">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2.5 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-xl shadow-lg shadow-violet-500/20">
            ...
            <div className="relative group">
              ...
              <div className="relative flex items-center bg-[#0a0a0f] border border-white/10 rounded-xl focus-within:border-violet-500/50 transition-colors">
                ...
                <input
                  ...
                  className="w-full bg-transparent py-3 pl-3 pr-4 text-sm text-gray-200 focus:outline-none placeholder-gray-600 font-medium"
                  value={searchQuery}
          ...
          {selectedPage ? (
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12 scroll-smooth">
              ...
              >
                ...
                {/* Contenu avec ReactMarkdown customisé pour utiliser CodeBlock */}
                <div className="min-h-[400px]">
                  {(!selectedPage.content || selectedPage.content.trim() === '') ? (
                    ...
                    <div className="prose prose-invert prose-violet max-w-none 
                      ...
                      prose-h1:text-3xl prose-h2:text-2xl prose-h2:mt-12 prose-h2:mb-6 prose-h2:border-b prose-h2:border-white/10 prose-h2:pb-2
                      prose-h3:text-xl prose-h3:text-violet-200 prose-h3:mt-8
                    ...
                    ">
                      <ReactMarkdown 
                        ...
                        components={{
                          code: CodeBlock,
                          // Hack pour éviter le double wrapping pre > div qui serait invalide en HTML
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-500 p-6">
              ...
              <p className="max-w-md text-center text-gray-400 leading-relaxed">
                Sélectionnez un nœud de données dans le panneau latéral pour commencer l'extraction.
              </p>
            </div>
          )}
        </main>
