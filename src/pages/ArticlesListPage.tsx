...
const getReadingTime = (description: string): number => {
  ...
  return 15; // valeur par défaut — sera affinée avec une colonne dédiée
};
...
const ArticleCard: React.FC<{ article: ArticleMetadata; onClick: () => void; index: number }> = ({ article, onClick, index }) => (
  <motion.div
    ...
    onClick={onClick}
    className="group relative bg-white dark:bg-[#13131a]/80 border border-gray-200 dark:border-white/5 rounded-2xl cursor-pointer hover:border-violet-500/50 hover:shadow-2xl hover:shadow-violet-500/10 transition-all duration-300 flex flex-col overflow-hidden"
  >
    {/* Accent gauche au hover */}
    <div className="absolute top-0 left-0 w-1 h-0 group-hover:h-full bg-violet-600 transition-all duration-300 rounded-l-2xl z-10" />
    ...
    <div className="flex items-center justify-between">
      <span className="px-2.5 py-1 bg-violet-500/10 border border-violet-500/20 text-violet-600 dark:text-violet-300 text-[10px] font-bold uppercase tracking-wider rounded-full">
        {article.category}
      </span>
    ...
    <div className="flex-1">
      <h3 className="font-bold text-lg text-gray-900 dark:text-white group-hover:text-violet-500 transition-colors leading-snug line-clamp-2 mb-2">
        {article.title}
      </h3>
      {article.description && (
    ...
    <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-white/5">
      <div className="flex flex-wrap gap-1.5">
        {article.tags?.slice(0, 2).map(tag => (
          <span key={tag} className="text-[9px] bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded text-gray-500 font-mono">
            #{tag}
          </span>
        ))}
      </div>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map(i => (
                <div key={i} className="h-64 bg-gray-100 dark:bg-white/5 animate-pulse rounded-2xl" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
