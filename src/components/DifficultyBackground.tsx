...
export const DifficultyBackground: React.FC<DifficultyBackgroundProps> = ({ difficulty = "default", tags = [] }) => {
  ...
  useEffect(() => {
    ...
    const createParticle = (isInitial = false): Particle => {
      ...
      // Pour Hard, on mélange flammes et cendres
      if (isHard && Math.random() > 0.6) {
          type = 'ash';
      }

      const x = Math.random() * canvas.width;
      let y = isInitial ? Math.random() * canvas.height : (type === 'flame' ? canvas.height + 20 : Math.random() * canvas.height);
      
      // Les cendres peuvent tomber du haut ou monter selon l'humeur
      if (!isInitial && type === 'ash') {
          y = Math.random() > 0.5 ? -20 : canvas.height + 20;
      }

      const r = (type === 'pixel' || type === 'ash') ? Math.random() * 2 + 1 : Math.random() * 3 + 1;
