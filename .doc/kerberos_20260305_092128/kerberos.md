...
* **[1. Fondations & Théorie de Kerberos](#partie-1-fondations-th-orie-de-kerberos)**
  * [Architecture & Concepts](#i-architecture-concepts)
  * [Les Vecteurs & Fonctionnement](#ii-les-vecteurs-fonctionnement)
  * [The Lab Setup](#iii-the-lab-setup)
  * [Pre-requisites & Reconnaissance](#iv-pre-requisites-reconnaissance)
* **[2. Implémentation & Défense](#partie-2-impl-mentation-d-fense)**
  * [Step-by-Step Implementation](#v-step-by-step-implementation)
  * [Vecteurs Modernes & Optimisations](#vi-vecteurs-modernes-optimisations)
  * [Troubleshooting](#vii-troubleshooting)
  * [Hardening & Blue Team](#viii-hardening-blue-team)
...
![Représentation macroscopique et abstraite de l'architecture Kerberos illustrant les flux de données chiffrées entre le KDC, les terminaux clients et les serveurs applicatifs. L'esthétique utilise des motifs de circuits intégrés et des nœuds de confiance interconnectés pour symboliser la centralisation des secrets au sein de la base de données NTDS.DIT](kerberos_macro_arch.png)
...
![Détail technique microscopique d'une structure de paquet Kerberos montrant les headers ASN.1, les champs de chiffrement EType 18 (AES256-CTS-HMAC-SHA1-96) et la structure binaire d'un Ticket-Granting Ticket encapsulant le Privilege Attribute Certificate](kerberos_packet_structure.png)
...
![Illustration conceptuelle montrant la vision d'un attaquant sur un réseau Active Directory : des fils d'or (SPN) relient les utilisateurs aux serveurs, tandis que des verrous chiffrés (Tickets) flottent au-dessus des flux réseau, certains apparaissant plus fragiles (RC4) que d'autres (AES)](attacker_recon_vision.png)
...
![Représentation abstraite d'une attaque par force brute offline : des cascades de caractères hexadécimaux se heurtent à un bouclier cryptographique (AES) tandis qu'un flux plus faible (RC4) se fissure sous la pression d'un cluster de calcul GPU](offline_cracking_metaphor.png)
...
![Architecture de défense multicouche montrant un Domain Controller fortifié, protégé par des pare-feu applicatifs et un centre de surveillance (SOC). Des flux de logs dorés (Audit) remontent vers une interface de monitoring tandis que des verrous AES-256 bloquent les tentatives d'accès non autorisées](hardening_view.png)
